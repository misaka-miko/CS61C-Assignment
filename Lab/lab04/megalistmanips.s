...
mapLoop:
    # FIX: the code only moves the address of the node to t1
    # add t1, s0, x0      # load the address of the array of current node into t1
    lw t1, 0(s0) # load the address of the current node into t1
    lw t2, 4(s0)        # load the size of the node's array into t2
    # NOTE: Now t1 holds the address of the array
    # t2 holds the size of the array

    # FIX: Wrong offset computing, it should be srl 2
    # add t1, t1, t0      # offset the array address by the count
    slli t3, t0, 2 # use temporary register t3 to store the calculated offset
    add t1, t1, t3
    lw a0, 0(t1)        # load the value at that address into a0

# FIX: Use temp registers after a function call without saving
    addi sp, sp, -12
    sw t0, 0(sp)
    sw t1, 4(sp)
    sw t2, 8(sp)
    ...
    # FIX: wrong usage of la
    # la a0, 8(s0)        # load the address of the next node into a0
    lw a0, 8(s0) # load the address of the next node into a0
    # FIX: we only need to mv the address value in s1 into a1, but not using lw
